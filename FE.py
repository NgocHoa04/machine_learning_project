...
class HanoiWeatherFE:
    """
    Thiết kế để dùng sau khi đã:
    - xử lý outlier
    - scale các biến (ví dụ: humidity_scale__humidity, scale_num__windspeed, ...)
    ...
    def __init__(
        ...
        spd_col: str = "scale_num__windspeed",
        lag_days: Sequence[int] = (1, 2, 3, 7),
        roll_windows: Sequence[int] = (3, 7, 14, 21, 30, 60, 90),
        calm_thr: float = 0.5,
    ...
    def _fe_wind_block(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.spd_col in out.columns:
            spd = out[self.spd_col].astype(float).fillna(0.0)
        else:
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 4) Bỏ helper columns không dùng
        for col in ["sunrise_dt", "sunset_dt"]:
