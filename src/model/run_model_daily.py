"""
Daily Temperature Prediction Model Training Pipeline

This production-ready script orchestrates the complete machine learning pipeline for
multi-horizon daily temperature forecasting using XGBoost models with walk-forward
...
Usage:
    python src/model/run_model_daily.py [options]
"""
...
class Config:
    ...
    # Model metadata
    MODEL_NAME = "Hanoi Daily Temp Predictor v1.0 (XGBoost)"
    MODEL_VERSION = "1.0.0"
    
    # File paths (relative to project root)
    DATA_FILE = "dataset/raw/Hanoi_Daily_Selected.csv"
...
def load_and_preprocess_data(
    ...
    logger.info(f"Loaded {len(df_daily):,} records")
    
    # Train-test split
    logger.info("Splitting data into train and test sets...")
    train_dataset, test_dataset = pipeline_module.train_test_split(df_daily)
    logger.info(f"Train set: {len(train_dataset):,} records")
    logger.info(f"Test set: {len(test_dataset):,} records")
    
    # Apply preprocessing pipeline
    logger.info("Applying preprocessing pipeline...")
    preprocessing_pipeline = pipeline_module.before_model_pipeline
    preprocessing_pipeline.fit(train_dataset)
    
    train_dataset = preprocessing_pipeline.transform(train_dataset)
    test_dataset = preprocessing_pipeline.transform(test_dataset)
...
def initialize_pipeline(
    ...
    import model.daily_model as daily_model
    
    logger.info("Creating multi-horizon XGBoost pipeline...")
    pipeline = daily_model.MultiHorizonWalkForwardOptuna_XGBoost_Pipeline(
        df=train_data,
...
def evaluate_models(
    ...
    for horizon, metrics in sorted(train_metrics.items()):
        ...
        logger.info(f"  Horizon {horizon}d | RMSE: {rmse:.4f} | MAE: {mae:.4f} | R²: {r2:.4f}")
...
def save_results(
    ...
    run_name = f"hanoi_daily_temp_v1_{timestamp}"
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train multi-horizon daily temperature prediction models",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python src/model/run_model_daily.py
  python src/model/run_model_daily.py --n-trials 200 --log-level DEBUG
  python src/model/run_model_daily.py --data-file dataset/raw/custom_data.csv
        """
    )
    ...
    parser.add_argument(
        ...
        help=f'Test size per fold in days (default: {Config.TEST_SIZE})'
    )
...
def main() -> int:
    ...
    try:
        ...
        logger.info("\n" + "="*80)
        logger.info("DAILY TEMPERATURE PREDICTION PIPELINE")
